...
icon = pygame.image.load("th.jpeg")
...
def game_intro():
   ...
   while intro:
      ...
      message_to_screen("Welcome to Tank Warl!", introcolor, -100, size="large")
...
def game_controls():
    ...
    while gcont:
     for event in pygame.event.get():
       ...
       message_to_screen("Move Turrent: Up and Down arrows",wheat, 10)
       message_to_screen("Move Tank: Left and Right arrows",wheat,50)
       message_to_screen("Press D to raise Power % AND Press A to Power % ", wheat,140)
...
def gameLoop():
   ...
   while not gameExit:
      ...
      if gameOver == True:
         message_to_screen("Green Over", red, -50, size="large")
      ...
      for event in pygame.event.get():
         ...
         if event.type == pygame.KEYDOWN:
            if event.key == pygame.K_LEFT:
               ...
               damage = fireShell(gun,tank, mainTankX, mainTankY, currentTurPos, fire_power, xlocation, barrier_width, randomHeight,enemyTankX,enemyTankY)
               ...
               for x in range(random.randrange(0,10)):
                  ...
                  if display_width * 0.3 > enemyTankX > display_width * 0.03:
                     ...
                     gameDisplay.fill(black)
                     health_bars(mainTankX, mainTankY, currentTurPos)
...
def pause():
   paused = True
   message_to_screen("Paused",introcolor, -100,size="larger")
...
def fireShell(xy,tankx,tanky,turPos,gun_power, xlocation, barrier_width, randomHeight,enemyTankX, enemyTankY):
   ...
   #fireshell()
def e_fireshell(xy, tankx, tanky, turPos, hun_power, xlocation, barrier_width, randomHeight, ptankx, ptanky):
   damage = 0
   currentPower = 1
   power_found = False
   ...
   while not power_found:
      ...
      check_y_1 = startingShell[0] <= display_height
      check_y_2 = startingShell[1] >= display_height - randomHeight
   ...
   while fire:
      ...
      if check_x_1 and check_x_2 and check_y_1 and check_x_2:
         hit_x = int((startingShell[0]))
         hit_y = int((startingShell[1]))
...
def enemy_tank(x,y, turPos):
   ...
   pygame.draw.line(gameDisplay, tankbottom, (x,y), possibleTurrets[turPos], turretWidth)
...
def you_win():
   ...
   while win:
      ...
      button("play Again", 150, 500, 150, 50, wheat, light_green, action="play")
      button("controls", 350,500,100,50,wheat,light_yellow, action="controls")
      button("quit", 550,500,100,wheat,light_red, action="quit")
